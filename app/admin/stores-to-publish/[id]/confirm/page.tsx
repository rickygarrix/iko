...
export default function StoreToPublishConfirmPage() {
  ...
  const buildOpeningHoursText = (list: string[]): string => {
    return list.map((time, idx) => {
      // すでに「月曜」などが付いていたらそのまま
      if (time.startsWith(DAYS[idx])) {
        return time;
      }
      // 「月」とだけ付いてる場合も「月曜」に直す
      if (time.startsWith(DAYS[idx][0])) {
        return `${DAYS[idx]} ${time.slice(1).trim()}`;
      }
      // それ以外は強制的に曜日を付与
      return `${DAYS[idx]} ${time.trim()}`;
    }).join("\n");
  };
  ...
  return (
    <div className="min-h-screen bg-[#FEFCF6] pt-24 px-10 pb-10 text-gray-800">
      <h1 className="text-2xl font-bold text-center mb-8">未公開店舗 確認画面</h1>
      ...
      <div className="max-w-2xl mx-auto space-y-4">
        ...
        <div>
          ...
          <p className="text-lg whitespace-pre-line">{buildOpeningHoursText(storeData.openingHoursList)}</p>
        </div>
        ...
        <div className="flex justify-center gap-4 mt-10">
          <button
            onClick={() => router.push(`/admin/stores-to-publish/${id}/edit`)}
